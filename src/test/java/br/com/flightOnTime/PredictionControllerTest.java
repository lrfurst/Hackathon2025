...
public class PredictionControllerTest {
    ...
    void predict_Success() throws Exception {
        // 1. O Mock deve retornar exatamente o que o teste vai verificar (Maiúsculo)
        PredictionResponseDTO mockResponse = new PredictionResponseDTO();
        mockResponse.setPrevisao("PONTUAL"); // <--- Alterado para MAIÚSCULO
        mockResponse.setProbabilidade(0.85);

        Mockito.when(predictionService.getPrediction(any(PredictionRequestDTO.class)))
                .thenReturn(mockResponse);

        // 2. O Request deve conter TODOS os campos obrigatórios (senão dá erro 400)
        PredictionRequestDTO request = new PredictionRequestDTO();
        request.setCompanhia("LATAM");
        ...
        mockMvc.perform(post("/predict")
                ...
                .andExpect(jsonPath("$.previsao").value("PONTUAL")) // <--- Agora vai bater!
